...
class HandEvaluator:
    ...
    def _standardize_cards(self, cards: List[Dict]) -> List[Tuple[str, str]]:
        ...
        for card in cards:
            ...
            if value == "?":
                logger.warning(f"Found card with '?' value: {card}")
                # Check if the card is likely a 7 of diamonds (common misclassification)
                if suit == "diamonds":
                    logger.info("Replacing unknown diamond card with 7 of diamonds")
                    value = "7"  # Replace with likely value
                else:
    ...
    def calculate_win_probability(self, 
        ...
        for card in std_community_cards:
            if card[0] in CARD_VALUES and card[1] in CARD_SUITS:
